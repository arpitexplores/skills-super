...
## Trend-To-Content Operating Loop

Use this when the user wants repeatable social output rather than a one-off post. The goal is not to automate everything blindly; it is to create a controlled content system with clear inputs, quality gates, scheduling rules, and performance feedback.

### Loop Structure

```text
Trend signals -> topic shortlist -> content pool -> brand/quality review -> platform formatting -> schedule -> publish/review -> performance learning
```

| Stage | Purpose | Output |
| --- | --- | --- |
| Signal capture | Identify topics worth reacting to or explaining | Trend list, source links, relevance notes |
| Topic shortlist | Remove weak, off-brand, or low-fit topics | Approved topic backlog |
| Content pool | Generate reusable drafts before scheduling | Draft posts by platform and pillar |
| Quality gate | Check accuracy, brand voice, risk, CTA, and usefulness | Approved, revise, or reject decision |
| Platform formatting | Adapt the same idea natively per channel | LinkedIn post, short post, carousel outline, caption |
| Scheduling | Place posts by priority, cadence, and audience timing | Calendar with owners and status |
| Measurement | Feed performance back into the next cycle | Keep, stop, test-next recommendations |

### Trend Signal Checklist

For each trend or topic, score:

| Check | Question |
| --- | --- |
| Relevance | Does this matter to the target audience? |
| Authority | Can the brand say something credible about it? |
| Timing | Is this fresh enough to be useful? |
| Differentiation | Do we have a specific angle, not just a summary? |
| Risk | Could this create factual, legal, reputational, or sensitivity issues? |
| Conversion fit | Can this naturally connect to a useful next step? |

Only create content from topics that pass relevance, authority, and risk checks.

### Content Pool Format

Maintain a content pool when producing posts in batches.

| Field | Purpose |
| --- | --- |
| `topic` | What the post is about |
| `source_or_input` | Source article, customer insight, campaign asset, or internal note |
| `pillar` | Content pillar supported by the post |
| `platform` | Channel the post is written for |
| `format` | Text post, thread, carousel, reel script, short video, story, poll |
| `hook` | First line or opening frame |
| `body` | Main draft |
| `cta` | Action requested from the audience |
| `asset_needed` | Image, chart, video, screenshot, link, none |
| `status` | Idea, draft, review, approved, scheduled, published, retired |
| `tracking_url` | Campaign or UTM link where relevant |
| `performance_note` | What happened after publishing |

### Review Gate

Before scheduling, verify:

- The post is factually safe and does not exaggerate claims.
- The angle is specific enough to avoid generic commentary.
- The tone fits the platform and brand.
- The CTA is appropriate for the audience's stage.
- Any image/video requirement is clear.
- Links and tracking are present where needed.
- Sensitive topics have human approval.
...
| Revise | Useful idea, but needs edits before scheduling |
| Reject | Off-brand, low-value, risky, or not worth publishing |

### Scheduling Discipline

When building a social schedule:

- Respect platform cadence and avoid flooding one channel.
- Space posts so similar topics do not compete with each other.
- Prioritise time-sensitive posts before evergreen posts.
- Keep enough evergreen content in reserve for slow news periods.
- Use dry-run or preview mode where tooling allows it.
- Require human approval before live posting for regulated, sensitive, or high-visibility brands.

### Performance Feedback

After publishing, classify each post:

| Result | Meaning | Next Action |
| --- | --- | --- |
| High reach, low action | Hook worked, offer/CTA did not | Improve CTA or destination |
| Low reach, high engagement | Strong niche value | Reuse angle, test better timing |
| High saves/shares | Useful reference content | Turn into carousel, guide, or email |
| High comments | Conversation trigger | Follow up with opinion, FAQ, or proof |
| Low everything | Weak topic, hook, or fit | Retire or rewrite from a stronger angle |

Feed these learnings into the next topic shortlist and content pool.
